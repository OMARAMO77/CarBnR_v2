...
print_message "Checking DNS propagation for www.eromo.tech..." "$GREEN"
if ! dig www.eromo.tech; then
    print_message "DNS lookup failed for www.eromo.tech." "$RED"
    exit 1
fi

# Obtain SSL certificate using Certbot with NGINX
print_message "Running Certbot to obtain SSL certificate..." "$GREEN"
if ! sudo certbot --nginx; then
    print_message "Certbot failed to obtain SSL certificate." "$RED"
    exit 1
fi

# Check SSL certificate symlinks
CERT_DIR="/etc/letsencrypt/live/www.eromo.tech/"
...
print_message "SSL certificate setup complete!" "$GREEN"
